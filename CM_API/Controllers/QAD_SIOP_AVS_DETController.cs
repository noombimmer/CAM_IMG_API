...
{
    public class QAD_SIOP_AVS_DETController : ApiController
    {
        private THAPP_TCOMMON_DEVEntities db = new THAPP_TCOMMON_DEVEntities();

        public IHttpActionResult Post(QAD_SIOP_AVS_DET param)
        {
            bool withCondition = false;
            string sqlSTring = "SELECT * FROM QAD_SIOP_AVS_DET WHERE 1 = 1 ";
            PropertyInfo[] tstRet = param.GetType().GetProperties();

            List<QAD_SIOP_AVS_DET> qAD_SIOP_AVS_DET = null;
            ...
            {
                qAD_SIOP_AVS_DET = db.QAD_SIOP_AVS_DET.SqlQuery(sqlSTring).ToList();
            }
            else
            {
                qAD_SIOP_AVS_DET = db.QAD_SIOP_AVS_DET.SqlQuery(sqlSTring).ToList();
            ...
            if (qAD_SIOP_AVS_DET == null)
            {
                return Ok(qAD_SIOP_AVS_DET);
            }
