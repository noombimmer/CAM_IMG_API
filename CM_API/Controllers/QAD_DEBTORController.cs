...
{
    public class QAD_DEBTORController : ApiController
    {
        private THAPP_TCOMMON_DEVEntities db = new THAPP_TCOMMON_DEVEntities();

        public IHttpActionResult Post(QAD_DEBTOR param)
        {
            bool withCondition = false;
            string sqlSTring = "SELECT * FROM QAD_DEBTOR WHERE 1 = 1 ";
            PropertyInfo[] tstRet = param.GetType().GetProperties();

            List<QAD_DEBTOR> qAD_DEBTOR = null;
            ...
            {
                qAD_DEBTOR = db.QAD_DEBTOR.SqlQuery(sqlSTring).ToList();
            }
            else
            {
                qAD_DEBTOR = db.QAD_DEBTOR.SqlQuery(sqlSTring).ToList();
